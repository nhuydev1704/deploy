...
function TabOrderList(props) {
    ...
    let columns = [
        ...columnOrders,
        {
            ...
            render: (value, row, index) => (
                <>
                    {row.paymentID && row.paymentID !== '' ? 'Paypal' : 'Tiền mặt'} ({' '}
        ...
        },
        {
            ...
            key: 'cart',
            align: 'center',
            render: (value, row, index) => <a onClick={() => handleCart(row.key)}>Xem chi tiết</a>,
        },
    ];
    ...
    let listOrder = [];
    console.log('🚀 ~ file: TabOrderList.js ~ line 84 ~ TabOrderList ~ dataPayment', dataPayment);
